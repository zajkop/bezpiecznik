...
| Indirect injection (LLM01) | ✅ instruction-in-data → execution detection + **`indirect-fuzz.json` (60)** wrapper × goal; plus the **web-side detector** that flags a *served* page carrying an instruction concealed from humans (comment / CSS-hidden / zero-width Unicode) aimed at any LLM agent that ingests it, plus a **ChatInject** detector for served content carrying forged chat-template role delimiters (see Web/API matrix) |
